(function () {
  ...
  style.textContent = `
    .legend-macd1{background:#72f2bd}.legend-macd2{background:#c891ff}.legend-macd3{background:#ff9d5c}
    .actual-return-line.macd1{stroke:#72f2bd}.actual-return-line.macd2{stroke:#c891ff;stroke-width:2.7}.actual-return-line.macd3{stroke:#ff9d5c;stroke-width:2.7;stroke-dasharray:8 4}
    .actual-dd-line.macd1{stroke:#f2c572}.actual-dd-line.macd2{stroke:#ff8bd1;stroke-dasharray:6 4}.actual-dd-line.macd3{stroke:#ffcd70;stroke-dasharray:2 4}
  ...
  if (comparisonTitle) comparisonTitle.textContent = "MACD 1·MACD 2·MACD 3 수익률 비교";
  const detailTitle = document.querySelector("#actual-equity-chart")?.closest(".live-results")?.querySelector(".detail-heading h3");
  if (detailTitle) detailTitle.textContent = "MACD 1·MACD 2·MACD 3 전략 상세 자산곡선";
  const legend = document.querySelector("#actual-equity-chart")?.previousElementSibling;
  if (legend?.classList.contains("chart-legend")) legend.innerHTML = `<span><i class="legend-macd1"></i>MACD 1 누적수익률</span><span><i class="legend-macd2"></i>MACD 2 누적수익률</span><span><i class="legend-macd3"></i>MACD 3 누적수익률</span><span><i class="legend-drawdown"></i>MACD 1 낙폭</span><span><i style="background:#ff8bd1"></i>MACD 2 낙폭</span><span><i style="background:#ffcd70"></i>MACD 3 낙폭</span>`;
  const scenario = document.createElement("div");
  scenario.className = "macd-scenario-note";
  scenario.innerHTML = `<div><strong>MACD 1 · 0선 돌파</strong>기존 RSI 30 초과 + MACD 0선 상향돌파 매수, 0선 하향돌파 매도 전략</div><div><strong>MACD 2 · MACD(12,26,9)</strong><div class="macd-scenario-grid"><span>0선 위·골든 <b>100%</b></span><span>0선 위·데드 <b>50%</b></span><span>0선 아래·골든 <b>50%</b></span><span>0선 아래·데드 <b>0%</b></span></div></div><div><strong>MACD 3 · MACD(18,39,9)</strong><div class="macd-scenario-grid"><span>0선 위·골든 <b>100%</b></span><span>0선 위·데드 <b>50%</b></span><span>0선 아래·골든 <b>50%</b></span><span>0선 아래·데드 <b>0%</b></span></div></div>`;
  ...
  window.drawActualDetail = function drawMacdComparison(result, market) {
    const strategies = ["MACD 1 · 0선 돌파", "MACD 2 · 0선×골든/데드 보유비중", "MACD 3 · MACD(18,39,9) 0선×골든/데드 보유비중"];
    ...
    const paths = entries.map((entry, index) => {
      const css = index === 0 ? "macd1" : index === 1 ? "macd2" : "macd3";
    ...
    svg.innerHTML = `<title>${market.replace("KRW-", "")} MACD 1·MACD 2·MACD 3 누적수익률 비교</title>${grids}<line class="actual-zero" x1="${left}" y1="${yReturn(0)}" x2="${right}" y2="${yReturn(0)}"/>${paths}<line class="chart-grid" x1="${left}" y1="${ddTop}" x2="${right}" y2="${ddTop}"/><text class="chart-tick" x="${left - 10}" y="${ddTop + 4}" text-anchor="end">0%</text><text class="chart-tick" x="${left - 10}" y="${ddBottom}" text-anchor="end">-${(maxDrawdown * 100).toFixed(0)}%</text>${dates}<text class="chart-value" x="${right}" y="22" text-anchor="end">${totals}</text>`;
    document.querySelector("#detail-reading").textContent = `${market.replace("KRW-", "")}의 동일 기간 MACD 1·MACD 2(12,26,9)·MACD 3(18,39,9) 누적수익률을 함께 표시합니다. 상태형 전략은 전일 확정 MACD 상태를 다음 일봉 시가부터 적용하며 수수료 0.05%와 슬리피지 0.08%를 반영했습니다.`;
  };
