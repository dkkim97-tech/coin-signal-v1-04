(function () {
  ...
  style.textContent = `
    .legend-macd1{background:#72f2bd}.legend-macd2{background:#c891ff}.legend-macd3{background:#ff9d5c}.legend-macd4{background:#4ca6ff}
    .actual-return-line.macd1{stroke:#72f2bd}.actual-return-line.macd2{stroke:#c891ff;stroke-width:2.7}.actual-return-line.macd3{stroke:#ff9d5c;stroke-width:2.7;stroke-dasharray:8 4}.actual-return-line.macd4{stroke:#4ca6ff;stroke-width:2.9;stroke-dasharray:12 4}
    .actual-dd-line.macd1{stroke:#f2c572}.actual-dd-line.macd2{stroke:#ff8bd1;stroke-dasharray:6 4}.actual-dd-line.macd3{stroke:#ffcd70;stroke-dasharray:2 4}.actual-dd-line.macd4{stroke:#ff5f7f;stroke-dasharray:10 4}
  ...
  if (isFutures) {
    ...
    if (subtitle) subtitle.textContent = "MACD 1·2·3 현물 기준 비교 · MACD 4: MACD(18,39,9), 0선 아래·데드크로스 숏 100% 선물 전략";
    const summaryTitle = document.querySelector(".live-results .chart-heading h2");
    if (summaryTitle) summaryTitle.textContent = "MACD 1·MACD 2·MACD 3·MACD 4 — 12종목 선물 전략 자산 비교 결과";
    const returnLegend = document.querySelector(".macd-return-legend");
    if (returnLegend && !returnLegend.querySelector(".macd4-color")) returnLegend.insertAdjacentHTML("beforeend", '<span><i class="macd4-color"></i>MACD 4 · MACD(18,39,9) 선물 롱/숏</span>');
  }

  const comparisonTitle = document.querySelector("#return-comparison")?.closest(".plot-block")?.querySelector("h3");
  if (comparisonTitle) comparisonTitle.textContent = isFutures ? "MACD 1·MACD 2·MACD 3·MACD 4 수익률 비교" : "MACD 1·MACD 2·MACD 3 수익률 비교";
  const detailTitle = document.querySelector("#actual-equity-chart")?.closest(".live-results")?.querySelector(".detail-heading h3");
  if (detailTitle) detailTitle.textContent = isFutures ? "MACD 1·MACD 2·MACD 3·MACD 4 전략 상세 자산곡선" : "MACD 1·MACD 2·MACD 3 전략 상세 자산곡선";
  const legend = document.querySelector("#actual-equity-chart")?.previousElementSibling;
  if (legend?.classList.contains("chart-legend")) legend.innerHTML = `<span><i class="legend-macd1"></i>MACD 1 누적수익률</span><span><i class="legend-macd2"></i>MACD 2 누적수익률</span><span><i class="legend-macd3"></i>MACD 3 누적수익률</span>${isFutures ? '<span><i class="legend-macd4"></i>MACD 4 선물 롱·숏 누적수익률</span>' : ""}<span><i class="legend-drawdown"></i>MACD 1 낙폭</span><span><i style="background:#ff8bd1"></i>MACD 2 낙폭</span><span><i style="background:#ffcd70"></i>MACD 3 낙폭</span>${isFutures ? '<span><i style="background:#ff5f7f"></i>MACD 4 낙폭</span>' : ""}`;
  const scenario = document.createElement("div");
  scenario.className = "macd-scenario-note";
  if (isFutures) scenario.classList.add("is-futures");
  scenario.innerHTML = `<div><strong>MACD 1 · 0선 돌파</strong>기존 RSI 30 초과 + MACD 0선 상향돌파 매수, 0선 하향돌파 매도 전략</div><div><strong>MACD 2 · MACD(12,26,9)</strong><div class="macd-scenario-grid"><span>0선 위·골든 <b>롱 100%</b></span><span>0선 위·데드 <b>롱 50%</b></span><span>0선 아래·골든 <b>롱 50%</b></span><span>0선 아래·데드 <b>0%</b></span></div></div><div><strong>MACD 3 · MACD(18,39,9)</strong><div class="macd-scenario-grid"><span>0선 위·골든 <b>롱 100%</b></span><span>0선 위·데드 <b>롱 50%</b></span><span>0선 아래·골든 <b>롱 50%</b></span><span>0선 아래·데드 <b>0%</b></span></div></div>${isFutures ? '<div><strong>MACD 4 · MACD(18,39,9) 선물</strong><div class="macd-scenario-grid"><span>0선 위·골든 <b>롱 100%</b></span><span>0선 위·데드 <b>롱 50%</b></span><span>0선 아래·골든 <b>롱 50%</b></span><span>0선 아래·데드 <b>숏 100%</b></span></div></div>' : ""}`;
  ...
  window.drawActualDetail = function drawMacdComparison(result, market) {
    const strategies = ["MACD 1 · 0선 돌파", "MACD 2 · 0선×골든/데드 보유비중", "MACD 3 · MACD(18,39,9) 0선×골든/데드 보유비중", ...(isFutures ? ["MACD 4 · MACD(18,39,9) 선물 롱/숏 보유비중"] : [])];
    ...
    const paths = entries.map((entry, index) => {
      const css = entry.strategy.startsWith("MACD 1") ? "macd1" : entry.strategy.startsWith("MACD 2") ? "macd2" : entry.strategy.startsWith("MACD 3") ? "macd3" : "macd4";
    ...
    const totals = entries.map((entry) => `${entry.strategy.split(" · ")[0]}: ${formatPercent(entry.summary.totalReturn)}`).join(" · ");
    svg.innerHTML = `<title>${market.replace("KRW-", "")} ${isFutures ? "MACD 1·MACD 2·MACD 3·MACD 4" : "MACD 1·MACD 2·MACD 3"} 누적수익률 비교</title>${grids}<line class="actual-zero" x1="${left}" y1="${yReturn(0)}" x2="${right}" y2="${yReturn(0)}"/>${paths}<line class="chart-grid" x1="${left}" y1="${ddTop}" x2="${right}" y2="${ddTop}"/><text class="chart-tick" x="${left - 10}" y="${ddTop + 4}" text-anchor="end">0%</text><text class="chart-tick" x="${left - 10}" y="${ddBottom}" text-anchor="end">-${(maxDrawdown * 100).toFixed(0)}%</text>${dates}<text class="chart-value" x="${right}" y="22" text-anchor="end">${totals}</text>`;
    document.querySelector("#detail-reading").textContent = `${market.replace("KRW-", "")}의 동일 기간 ${isFutures ? "MACD 1·MACD 2(12,26,9)·MACD 3(18,39,9)·MACD 4(18,39,9 선물 롱/숏)" : "MACD 1·MACD 2(12,26,9)·MACD 3(18,39,9)"} 누적수익률을 함께 표시합니다. 상태형 전략은 전일 확정 MACD 상태를 다음 일봉 시가부터 적용하며 수수료 0.05%와 슬리피지 0.08%를 반영했습니다.${isFutures ? " MACD 4는 0선 아래·데드크로스에서 1배 숏 100%를 적용합니다." : ""}`;
  };
