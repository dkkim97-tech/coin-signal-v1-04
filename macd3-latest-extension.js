(function () {
  ...
  const macd3Name = "MACD 3 · MACD(18,39,9) 0선×골든/데드 보유비중";
  const feeRate = 0.0005;
  const slippage = 0.0008;
  const baseRefresh = window.refreshUpbitBacktestFromLatestData;
  if (typeof baseRefresh !== "function" || !window.__MACD3_RESULT__) return;
  ...
  window.refreshUpbitBacktestFromLatestData = async function refreshAllMacdStrategies() {
    ...
    const updatedMacd3 = await extendAll(window.__MACD3_RESULT__, response.latestCandlesByMarket);
    response.result.__macd3Applied = true;
    response.result.summaries.push(...updatedMacd3.summaries);
    response.result.series.push(...updatedMacd3.series);
    return response;
  };
  ...
  async function extendAll(base, sharedCandles = {}) {
    ...
    for (const baseSeries of base.series) {
      ...
      if (!candles.length) {
        summaries.push({ ...baseSummary, strategy: macd3Name });
        seriesRows.push({ ...baseSeries, strategy: macd3Name });
        continue;
      }
  ...
  async function fetchDailyCandles(market) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (response.status !== 429 || attempt === 2) throw new Error(`${market} MACD 3 최신 일봉 연결 실패 (${response.status})`);
