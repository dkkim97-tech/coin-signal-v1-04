(function () {
  ...
  const macd4Name = "MACD 4 · MACD(18,39,9) 선물 롱/숏 보유비중";
  const feeRate = 0.0005;
  const slippage = 0.0008;
  const baseRefresh = window.refreshUpbitBacktestFromLatestData;
  if (!isFutures || typeof baseRefresh !== "function" || !window.__MACD4_RESULT__) return;

  window.refreshUpbitBacktestFromLatestData = async function refreshAllMacdStrategies() {
    const response = await baseRefresh();
    for (const row of response.result.summaries || []) if (!row.strategy) row.strategy = macd1Name;
    for (const row of response.result.series || []) if (!row.strategy) row.strategy = macd1Name;
    const updatedMacd4 = await extendAll(window.__MACD4_RESULT__, response.latestCandlesByMarket);
    response.result.__macd4Applied = true;
    response.result.summaries.push(...updatedMacd4.summaries);
    response.result.series.push(...updatedMacd4.series);
    return response;
  };
  ...
  async function extendAll(base, sharedCandles = {}) {
    ...
    for (const baseSeries of base.series) {
      ...
      if (!candles.length) {
        summaries.push({ ...baseSummary, strategy: macd4Name });
        seriesRows.push({ ...baseSeries, strategy: macd4Name });
        continue;
      }
      ...
      summaries.push({ ...baseSummary, strategy: macd4Name, to: new Date(newest).toISOString(), totalReturn: extension.finalEquity / baseSeries.initialEquity - 1, maxDrawdown: Math.max(baseSummary.maxDrawdown, ...extension.equityCurve.map((point) => point.drawdown)), completedTrades: trades.length, winRate: trades.length ? wins.length / trades.length : 0, profitFactor: grossLoss ? grossProfit / grossLoss : null, grossProfit, grossLoss });
      seriesRows.push({ ...baseSeries, strategy: macd4Name, finalEquity: extension.finalEquity, finalCash: extension.cash, finalQuantity: extension.quantity, finalExposure: extension.exposure, peakEquity: extension.peak, equityCurve: [...baseSeries.equityCurve, ...extension.equityCurve], trades });
    }
  ...
  async function fetchDailyCandles(market) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (response.status !== 429 || attempt === 2) throw new Error(`${market} MACD 4 최신 일봉 연결 실패 (${response.status})`);
  ...
  function extendSeries(base, candles) {
    ...
    for (let index = 1; index < candles.length; index += 1) {
      ...
      if (target !== exposure) {
        ...
        trades.push({ market: base.market, timestamp: candle.timestamp, entryTime: segmentStartTime, price: candle.open, pnl: before - segmentStartEquity, returnPct: segmentStartEquity ? before / segmentStartEquity - 1 : 0, reason: target < 0 ? "0선 아래·데드크로스: 숏 100%" : `롱 보유 비중 ${(target * 100).toFixed(0)}%로 변경`, side: exposure < 0 ? "short" : "long", type: "allocation" });
  ...
  function rebalance(cash, quantity, target, rawPrice) {
    ...
    const deltaQuantity = (equity * target) / rawPrice - quantity;
    if (deltaQuantity > 0) {
      const executionPrice = rawPrice * (1 + slippage), notional = deltaQuantity * executionPrice;
      return { cash: cash - notional * (1 + feeRate), quantity: quantity + deltaQuantity };
    }
    if (deltaQuantity < 0) {
      const units = -deltaQuantity, executionPrice = rawPrice * (1 - slippage), proceeds = units * executionPrice;
      return { cash: cash + proceeds * (1 - feeRate), quantity: quantity - units };
    }
  ...
  function targetExposure(line, signal) {
    ...
    return line >= 0 ? (line >= signal ? 1 : 0.5) : (line >= signal ? 0.5 : -1);
  }
