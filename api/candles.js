const TIMEFRAMES = {
  ...
  week: { endpoint: "weeks", milliseconds: 604800000 },
...
export default async function handler(request, response) {
  ...
  const requestedCount = Math.min(2000, Math.max(80, Math.floor(Number(request.query.count) || 200)));
  ...
  try {
    ...
    while (rows.size < fetchTarget) {
      ...
      const upstream = await fetch(`https://api.upbit.com/v1/candles/${config.endpoint}?${query}`, { signal: AbortSignal.timeout(8000), headers: { Accept: "application/json", "User-Agent": "coin-signal-v1.16" } });
      ...
      for (const candle of page) {
        ...
        rows.set(timestamp, { timestamp, open: Number(candle.opening_price), high: Number(candle.high_price), low: Number(candle.low_price), close: Number(candle.trade_price), volume: Number(candle.candle_acc_trade_volume) });
      }
    ...
    const candles = [...rows.values()].filter((candle) => candle.timestamp + config.milliseconds <= Date.now() && candle.open > 0 && candle.high > 0 && candle.low > 0 && candle.close > 0).sort((a, b) => a.timestamp - b.timestamp).slice(-requestedCount);
